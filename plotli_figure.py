...
def filter_data(dataframe,num_period):
    if num_period == '1mo':
        ...
        date=dataframe.index[-1]+dateutil.relativedelta.relativedelta(day=-5)
        ...
        date=datetime.datetime(dataframe.index[-1].year,1,1).strframe('%Y-%m-%d')
    else:
...
def candlestick(dataframe,num_period):
    ...
    fig.update_layout(showlegend=False,height=500,margin=dict(l=0,b=0,r=20,t=20),plot_bgcolor='white',paper_bgcolor='#e1efff')
    return fig
...
def Moving_average(dataframe,num_period):
    ...
    fig.update_xaxes(rangeslider_visible=True)
    fig.update_layout(height=500,margin=dict(l=0,r=0,t=20,b=0),plot_bgcolor='white',paper_bgcolor='#e1efff',legend=dict(yanchor='top',xanchor='right'))
