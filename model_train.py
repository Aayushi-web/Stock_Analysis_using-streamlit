...
def get_forecast(original_price,differening_order):
    predictions=fit_model(original_price,differening_order)
...
def evaluate_model(original_price, differencing_order):
    train_data, test_data = original_price[:-30], original_price[-30:]  # ✅ Fix indexing

    predictions = fit_model(train_data, differencing_order)

    # Ensure `test_data` and `predictions` are the same length
    predictions = predictions[:len(test_data)]  # ✅ Trim predictions if needed
