...
from pages.utils.plotli_figure import filter_data,plotly_table,close_chart,candlestick,RSI,Moving_average,MACD
st.set_page_config(
layout="wide"
...
col1.metric("Daily Change", 
            str(round(data["Close"].iloc[-2].item(), 2)), 
            str(round(daily_change.item(), 2)))
last_days_df=data.tail(10).sort_index(ascending=False).round(3)
fig_df=plotly_table(last_days_df)
